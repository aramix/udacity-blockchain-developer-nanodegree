  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        block.timestamp = parseInt(Date.now().toString().slice(0, -3));
        if (self.height > 0) {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if ((currentTime - messageTime) / 60 >= 5) throw new Error('Message is older then 5 minutes!');
        ...
        if (await block.validate()) {
          ...
          if (block.previousBlockHash) {
            if (self.chain[block.height - 1].hash !== block.previousBlockHash) {
              errorLog.push(`Invalid previousBlockHash for block with height ${block.height}`);
            }
          }
        }
      });
